...
const LAYER_META: Record<string, { label: string; icon: any; color: string }> = {
  onchain: { label: '链上', icon: Link2, color: 'purple' },
  technical: { label: '技术', icon: Activity, color: 'blue' },
  macro: { label: '宏观', icon: Globe, color: 'amber' },
};
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between text-xs">
        <span className="text-red-400">极空</span>
        <span className="text-gray-500">中性</span>
        <span className="text-emerald-400">极多</span>
      </div>
...
function LayerBar({ layer, score }: { layer: string; score: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[11px] text-gray-400 w-8 shrink-0">{meta.label}</span>
    ...
    <div className="flex items-start gap-2 py-1.5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[9px] px-1 py-0.5 rounded bg-gray-800 text-gray-500`}>{meta.label}</span>
          <span className="text-[10px] text-gray-600">强度 {signal.strength}</span>
        </div>
...
function AssetCard({ fused }: { fused: FusedSignal }) {
  ...
  const dirLabel = fused.direction === 'bullish' ? '偏多' : fused.direction === 'bearish' ? '偏空' : '震荡';
  ...
  return (
    <div className={`bg-gradient-to-br ${dirBg} to-transparent border ${dirColor} rounded-xl overflow-hidden`}>
      ...
      <button onClick={() => setExpanded(!expanded)}
        ...
        {expanded ? '收起' : `展开 ${fused.signals.length} 个信号`}
      </button>
...
export default function SignalsPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-950 pt-20 flex items-center justify-center">
      <div className="animate-pulse text-gray-500">聚合信号中...</div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-950 pt-20 pb-12 px-4">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-2xl font-bold text-gray-100">信号聚合器</h1>
            </div>
            <p className="text-sm text-gray-500 mt-1">链上 × 技术 × 宏观 — 三层过滤，一个结论</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
        ...
        <div className="flex items-center gap-4 text-xs text-gray-500">
          <span className="flex items-center gap-1"><BarChart3 className="w-3 h-3" /> {data.totalSignals} 个信号源</span>
          <span>3 层过滤</span>
          <span>{data.fused.length} 个标的</span>
        </div>
        ...
        <div className="bg-gray-900/30 border border-gray-800/30 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-gray-300 mb-3">📐 融合方法论</h3>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-purple-400 font-bold text-lg">35%</div>
              <div className="text-xs text-gray-400">链上数据</div>
              <div className="text-[10px] text-gray-600 mt-1">F&G · 鲸鱼 · 费率 · 稳定币</div>
            </div>
            <div>
              <div className="text-blue-400 font-bold text-lg">35%</div>
              <div className="text-xs text-gray-400">技术分析</div>
              <div className="text-[10px] text-gray-600 mt-1">趋势 · RSI · 量能 · 关键位</div>
            </div>
            <div>
              <div className="text-amber-400 font-bold text-lg">30%</div>
              <div className="text-xs text-gray-400">宏观环境</div>
              <div className="text-[10px] text-gray-600 mt-1">ETF · VIX · 美元 · 地缘</div>
            </div>
          </div>
        </div>
      </div>
